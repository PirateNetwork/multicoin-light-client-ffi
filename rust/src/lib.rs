...
pub extern "C" fn zcashlc_init_accounts_table_with_keys(
    ...
    let res = catch_panic(|| {
        ...
        let mut ufvks: Vec<UnifiedFullViewingKey> = Vec::new();
...
fn unified_viewing_key_new(ufvk: &UnifiedFullViewingKey, network: Network) -> FFIUnifiedViewingKey {
            ...
            .map(|account| {
                UnifiedSpendingKey::from_seed(&network, &seed, AccountId::from(account))
                    ...
                    .map(|usk| {
                        ...
                        unified_viewing_key_new(&ufvk, network)
                    })
            })
            .collect::<Result<_, _>>()?;
